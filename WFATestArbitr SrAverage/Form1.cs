...
namespace WFATestArbitr
{
    public partial class Form1 : Form
    {
        ...
        public MemoryMappedFile MemoryTerminalSendCommandToCsharp;
        // Создает поток для чтения
        StreamReader SR_TerminalSendCommandToCsharp;
        // Создает поток для записи
        StreamWriter SW_TerminalSendCommandToCsharp;
        ...
        public bool succesOrder1 = false;
        public bool succesOrder2 = false;
        ...
        public double stopBasisBySpread = 0;
        public double stopBasisSellSpread = 0;



        public List<double> tekBasisByList = new List<double>();
        public List<double> tekBasisSellList = new List<double>();
        public int tekBasisListMax = 10000;  
        ...
        private void SpredCalc()
        {
            ...
            if (tekBid1 > 0 && tekBid2 > 0 && tekOffer1 > 0 && tekOffer2 > 0)
            {
                ...
                if (tekBasisSellSpread > 0 && tekBasisBySpread > 0)
                {
                    ...
                    {
                        tekBasisByList.Add(tekBasisBySpread);
                        tekBasisSellList.Add(tekBasisSellSpread);

                        if (tekBasisListMax <= tekBasisByList.Count)
                        {
                            tekBasisByList.RemoveRange(0, Convert.ToInt16(tekBasisListMax / 2));
                            tekBasisSellList.RemoveRange(0, Convert.ToInt16(tekBasisListMax / 2));
                            trigger = false;
                            
                        }
                        double sredneeBuff = 0;

                        for (int i = 0; i < tekBasisByList.Count; i++)
                        {
                            if (max < tekBasisByList[i]) { max = tekBasisBySpread; };
                            if (min > tekBasisSellList[i]) { min = tekBasisSellSpread; };
                            sredneeBuff += min + (max - min) / 2;
                            sredneeBuff /= 2;
                        ...
                        stopBasisBySpread = max + Convert.ToInt16(this.textBoxMinKol.Text);
                        stopBasisSellSpread = min - Convert.ToInt16(this.textBoxMinKol.Text);
                        ...
                        this.textBoxStopBuy.Text = stopBasisBySpread.ToString();
                        this.textBoxStopSell.Text = stopBasisSellSpread.ToString();
                        this.textBoxMax.Text = max.ToString();
                        this.textBoxMin.Text = min.ToString();
                        labelTrigger.Text = trigger.ToString();
                    ...
                    try
                    {
                        ...
                        if (trigger == false)
                        {
                            if (tekBasisBySpread >= priceBuyBasis && status != "BuyBasis Open" && status != "SellBasis Open" && checkBoxBuyBasis.Checked)
                            {
                                //Покупаем базис
                            ...
                            if (tekBasisSellSpread <= priceSellBasis && status != "BuyBasis Open" && status != "SellBasis Open" && checkBoxSellBasis.Checked)
                            {
                                //Продаем базис
                                if (priceBuyBasis - priceSellBasis >= Convert.ToDouble(this.textBoxMinKol.Text))
                                {
                        ...
                        else if(trigger == true)
                        {
                            ...
                            //Выход по стопу завершаем работу робота
                            if (tekBasisBySpread>= stopBasisBySpread && status == "BuyBasis Open")
                            {
                                SetQUIKCommandData("BuyBasis Close");
                                status = "BuyBasis Close End Work";
                                this.labelStatus.Text = status;
                                trigger = false;
                                summ += basisBuyBuff - tekBasisSellSpread;
                                Run = false;
                            ...
                            //   stopBasisBySpread
                            //Выход по стопу завершаем работу робота
                            if (tekBasisSellSpread <= stopBasisSellSpread && status == "SellBasis Open")
                            {
                                SetQUIKCommandData("SellBasis Close");
                                status = "SellBasis Close End Work";
                                this.labelStatus.Text = status;
                                trigger = false;
                                summ += tekBasisBySpread - basisSellBuff;
                                Run = false;
        ...
        public Form1()
        {
            ...
            //SendCommandToCsharp
            MemoryTerminalSendCommandToCsharp = MemoryMappedFile.CreateOrOpen("TerminalSendCommandToCsharp", 256, MemoryMappedFileAccess.ReadWrite);
            SR_TerminalSendCommandToCsharp = new StreamReader(MemoryTerminalSendCommandToCsharp.CreateViewStream(), System.Text.Encoding.Default);
            SW_TerminalSendCommandToCsharp = new StreamWriter(MemoryTerminalSendCommandToCsharp.CreateViewStream(), System.Text.Encoding.Default);
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            settingsStr.Add(this.textBoxStopBuy.Text);
            settingsStr.Add(this.textBoxStopSell.Text);

            settingsStr.Add(this.labelStatus.Text);

            settingsStr.Add(trigger.ToString());
            settingsStr.Add(checkBoxBuyBasis.Checked.ToString());
            settingsStr.Add(checkBoxSellBasis.Checked.ToString());
            settingsStr.Add(this.textBoxTimeRasch.Text);

            settingsStr.Add(checkBoxAuto.Checked.ToString());
            settingsStr.Add(labelSumm.Text);
            settingsStr.Add(labelPosInstr1.Text);
            settingsStr.Add(labelPosInstr2.Text);
        ...
        {
            try {
               tekBasisListMax = Convert.ToInt16( this.textBoxTimeRasch.Text);
            } catch  { }
        }
        ...
        private void Form1_Shown(object sender, EventArgs e)
        {
            ...
            this.textBoxStopBuy.Text = settingsStr[4];
            this.textBoxStopSell.Text = settingsStr[5];

            this.labelStatus.Text = settingsStr[6];
            status = settingsStr[6];

            trigger = Convert.ToBoolean(settingsStr[7]);

            this.checkBoxBuyBasis.Checked= Convert.ToBoolean(settingsStr[8]);
            this.checkBoxSellBasis.Checked = Convert.ToBoolean(settingsStr[9]);

            this.textBoxTimeRasch.Text = settingsStr[10];

            tekBasisListMax = Convert.ToInt16(settingsStr[10]) * 2;
            checkBoxAuto.Checked = Convert.ToBoolean(settingsStr[11]);
            labelSumm.Text = settingsStr[12];
            summ = Convert.ToInt16(settingsStr[12]);
            labelPosInstr1.Text = settingsStr[13];
            labelPosInstr2.Text = settingsStr[14];
            //  settingsStr.Add(checkBoxAuto.Checked.ToString());
            //   settingsStr.Add(labelSumm.Text);
            //   settingsStr.Add(labelPosInstr1.Text);
            //   settingsStr.Add(labelPosInstr2.Text);
            ...
            max = Convert.ToDouble(this.textBoxMax.Text);
            min = Convert.ToDouble(this.textBoxMin.Text);
          //  srednee = Convert.ToDouble(this.textBoxSred.Text);

            priceBuyBasis = (max - srednee) / 2 + srednee;
            priceSellBasis = min + (srednee - min) / 2;
            stopBasisBySpread = max + priceBuyBasis - priceSellBasis;
            stopBasisSellSpread = min - priceBuyBasis - priceSellBasis;
